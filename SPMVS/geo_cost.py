...
class GeoUCSNet(nn.Module):
    def __init__(self, geo_model, lamb=1.5, stage_configs=[64, 32, 8], grad_method="detach", base_chs=[8, 8, 8],
                 feat_ext_ch=8, use_qc=False, img_channel=3):
        super(GeoUCSNet, self).__init__()
        ...
        self.feature_extraction = FeatureNet(base_channels=feat_ext_ch, num_stage=self.num_stage, in_channels=img_channel)
    ...
    def forward(self, imgs, proj_matrices, depth_values):
        ...
        # depth_min = torch.tensor(self.height_range[0], device=depth_values.device, dtype=torch.float32)
        # depth_max = torch.tensor(self.height_range[1], device=depth_values.device, dtype=torch.float32)
