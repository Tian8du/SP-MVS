...
from modules.Visual import  normalize_image, plot_WHUTLC_save

cudnn.benchmark = True


parser = argparse.ArgumentParser(description='A PyTorch Implementation')
parser.add_argument('--model', default="casmvs", help='select model', choices=['samsat', 'red', "casmvs", "ucs", "emvs","eucs"])
...
parser.add_argument('--dataset_root', default=r'H:\data\WHU_TLC\WHU-TLC\open_dataset_rpc\test', help='dataset root')

parser.add_argument('--loadckpt', default=r"E:\Codes2\EMVS-Net\checkpoints\casmvs\rpc\model_000020.ckpt",
...
def predict():
    ...
    if args.model == "casmvs":
        model = CascadeMVSNet(min_interval=args.min_interval,
                              ...
                              geo_model=args.geo_model, use_qc=args.use_qc,img_channel=1)
        ...
        model = ST_SatMVS(min_interval=args.min_interval,
                          ...
                          depth_interals_ratio=[float(d_i) for d_i in args.depth_inter_r.split(",") if d_i],
                          cr_base_chs=[int(ch) for ch in args.cr_base_chs.split(",") if ch],
                          geo_model=args.geo_model, use_qc=args.use_qc, img_channel=3)
    ...
    for batch_idx, sample in enumerate(Pre_ImgLoader):
        ...
        # plt.show()
        output_path = os.path.join("./ResultsWHUTLC",bname)
        output_path = output_path

        # plot_all_images2(depth_gt, depth_est, normalize_image(first_image), prob, save_path=output_path)
        # plot_US3D_save(depth_gt, depth_est, normalize_image(first_image), prob, output_path, model=args.model,save_images=["est"])
        plot_WHUTLC_save(depth_gt, depth_est, normalize_image(first_image), prob, output_path, model=args.model,
                       save_images=[ "blur"])
